...
var CompositeScriptHashInput = Input.CompositeScriptHash;
...
Transaction.prototype.fromComposite = function(utxo, compinputs, threshold) {
$.checkArgument(BufferUtil.isBuffer(compinputs));
  if (_.isArray(utxo)) {
    var self = this;
    _.each(utxo, function(utxo) {
      self.fromComposite(utxo, compinputs, threshold);
    });
    return this;
  }
  var exists = _.some(this.inputs, function(input) {
  ...
  if (compinputs ) {
    this._fromCompositeUtxo(utxo, compinputs, threshold);
  }
  return this;
};
...
// I the utxo, spendable by multisig
...
Transaction.prototype._fromMultisigUtxo = function(utxo, pubkeys, threshold) {
  ...
  var clazz;
  utxo = new UnspentOutput(utxo);
  if (utxo.script.isMultisigOut()) { // If there is a command of multisig, p2pkh type
    clazz = MultiSigInput;
  } else if (utxo.script.isScriptHashOut()) {
    clazz = MultiSigScriptHashInput;   // It is a script hash, already knows it is multisig. So use that object
  } else {
    throw new Error("@TODO");
  }
  this.addInput(new clazz({   // Passing arguments for one of the above class
    output: new Output({
      script: utxo.script,
      satoshis: utxo.satoshis
    }),
...
Transaction.prototype._fromCompositeUtxo = function(utxo, compinputs, threshold) {
  var clazz;
  utxo = new UnspentOutput(utxo);

  if (utxo.script.isScriptHashOut()) {
    clazz = CompositeScriptHashInput;
  } else {
    throw new Error("@TODO");
  }
  this.addInput(new clazz({
...
Transaction.prototype.addInput = function(input, outputScript, satoshis) {
  $.checkArgumentType(input, Input, 'input'); // Check if it is input class

  // input.output has to be present or other two
  ...
  if (!input.output && outputScript && !_.isUndefined(satoshis)) {
    ...
    $.checkArgumentType(satoshis, 'number', 'satoshis'); // checks if satoshi is number
