...
Script.buildCompositeOut = function(compinput, threshold, opts) {
  opts = opts || {}; // not used
...
Script.buildP2SHCompositeIn = function(compinput, threshold, opts) {
  $.checkArgument(BufferUtil.isBuffer(compinput));
  opts = opts || {};
  var s = new Script();
  s.add(compinput);
  s.add(Script.buildCompositeOut(compinput, threshold, opts).toBuffer());
  return s;
};
